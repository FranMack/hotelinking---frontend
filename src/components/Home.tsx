...
export function Home(){
    ...
    const getDiscountHandler= async (productId:string)=>{
        ...
        try{
            const newCupon = await axios.post(`http://localhost:3000/api/user/add-discount`,{productId:productId},{  headers: {
    ...
    <main className="home-container">
        {openPopUp && <PopUp handlePopUp={handlePopUp} message={popUpMessage} handleNavigate={togleMenu}/>}
{menuOpen&& <Login/>}
{menuRegisterOpen && <Register handlePopUp={handlePopUp} message="A continuación recibirá un email que le permitira validar su direccion de correo"/>}
        <div className="anuncio-container">
            <img className="anuncio-background" src={banner} alt="Banner promocional" />
        ...
        <div className="grid-container">
            {productList.map((product:Product)=>{
                return(<div className="grid-card" key={product.id}>
                    <img src={product.img} alt={product.name} />
                    <div className="descuento-container">
                        <p>{`CUPON % ${product.discount} OFF`}</p>
                    </div>
                    ...
                    <button onClick={()=>getDiscountHandler(product.id)} className="button-cupon"> Obtener cupon</button>
